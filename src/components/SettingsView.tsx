...
export default function SettingsView({
  ...
  const [pushNotif, setPushNotif] = useState(() => {
    const saved = localStorage.getItem('pushNotif');
    return saved !== null ? saved === 'true' : true;
  });
  const [toastNotif, setToastNotif] = useState(() => {
    const saved = localStorage.getItem('toastNotif');
    return saved !== null ? saved === 'true' : true;
  });
  ...
  useEffect(() => {
    localStorage.setItem('pushNotif', String(pushNotif));
  }, [pushNotif]);

  useEffect(() => {
    localStorage.setItem('toastNotif', String(toastNotif));
  }, [toastNotif]);

  useEffect(() => {
    localStorage.setItem('soundNotif', String(soundNotif));
    // Synced with TTS output setting
    localStorage.setItem('smarthome_voice_tts_enabled', String(soundNotif));
  }, [soundNotif]);
  ...
  // Google Home Free Sync Configuration States
  ...
  useEffect(() => {
    fetch('/api/local-lan/config')
      ...
      .then(data => {
        setLocalConfigState({
          googleHomeEmail: data.googleHomeEmail || '',
          googleHomeLocalIP: data.googleHomeLocalIP || '',
          googleHomeSyncToken: data.googleHomeSyncToken || '',
          connected: data.connected || false
        });
      })
      .catch(err => console.error('Error loading Google Home config:', err));
  }, []);
  ...
  const handleScanLocalDevices = async () => {
    setIsSyncing(true);
    setSyncStatus(language === 'tr' ? 'Google Home cihazlarınız yerel/bulut entegrasyonuyla ücretsiz eşitleniyor...' : 'Syncing Google Home devices via local & cloud free integration...');
    try {
      const res = await fetch('/api/local-lan/scan', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        setSyncStatus(data.log || (language === 'tr' ? 'Eşitleme başarıyla tamamlandı!' : 'Sync completed successfully!'));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        {/* Google Home Integration Details Card */}
        <div className="p-6 rounded-3xl bg-white dark:bg-polish-dark-card border border-slate-200/60 dark:border-slate-800/80 shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-sm font-bold font-display text-slate-800 dark:text-slate-100">
                  {language === 'tr' ? 'Google Home Entegrasyonu (Ücretsiz Yerel Köprü)' : 'Google Home Integration (Free Local Bridge)'}
                </h3>
                <p className="text-[10px] text-slate-400">
                  {language === 'tr' ? 'Cihaz durumlarını ve tanımlarını Google Home üzerindeki odalarınızla ücretsiz eşitleyin' : 'Sync device states and names with your Google Home rooms completely for free'}
                </p>
              </div>
            </div>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${localConfigState.connected ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-800/50'}`}>
              {localConfigState.connected ? (language === 'tr' ? 'Bağlı' : 'Connected') : (language === 'tr' ? 'Beklemede' : 'Idle')}
            </span>
          </div>
          ...
          <form onSubmit={handleSaveLocalConfig} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-bold text-slate-400">
                {language === 'tr' ? 'Google Home E-Posta' : 'Google Home E-Mail'}
              </label>
              <input
                type="text"
                placeholder="ornek@gmail.com"
                value={localConfigState.googleHomeEmail}
                onChange={e => setLocalConfigState({ ...localConfigState, googleHomeEmail: e.target.value })}
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-bold text-slate-400">
                {language === 'tr' ? 'Yerel Köprü IP (Opsiyonel)' : 'Local Bridge IP (Optional)'}
              </label>
              <input
                type="text"
                placeholder="192.168.1.100"
                value={localConfigState.googleHomeLocalIP}
                onChange={e => setLocalConfigState({ ...localConfigState, googleHomeLocalIP: e.target.value })}
                className="w-full px-3 py-2 rounded-xl border border-slate-200/60 dark:border-slate-800 bg-slate-50/50 dark:bg-polish-dark-header text-xs text-slate-700 dark:text-slate-200 font-sans focus:border-indigo-500 focus:outline-hidden"
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-bold text-slate-400">
                {language === 'tr' ? 'Eşitleme Anahtarı (Token)' : 'Sync Token / Code'}
              </label>
              <input
                type="password"
                placeholder="••••••••••••••••••••••••"
                value={localConfigState.googleHomeSyncToken}
                onChange={e => setLocalConfigState({ ...localConfigState, googleHomeSyncToken: e.target.value })}
            ...
            <div className="sm:col-span-3 flex flex-wrap items-center justify-between gap-3 pt-2">
              <div className="flex items-center gap-1 text-[10px] text-slate-400">
                ...
                <span>{language === 'tr' ? 'Google Home mobil uygulamasındaki gibi cihazları ve anlık durumları bulut ücreti ödemeden çeker.' : 'Fetches devices and status in real-time without paying any cloud/developer fee.'}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {language === 'tr' ? 'Google Home Cihazlarını Eşitle' : 'Sync Google Home Devices'}
                </button>
              </div>
            </div>
          </form>
